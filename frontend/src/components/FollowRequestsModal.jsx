...
import { fetchFollowRequests, acceptFollowRequest, declineFollowRequest } from "../api.js";
import { useI18n } from "../i18n/I18nContext.jsx";
import Avatar from "./Avatar.jsx";
import { RowSkeletonList } from "./Skeleton.jsx";

export default function FollowRequestsModal({ onClose, onChange }) {
  const { t } = useI18n();
    ...
    <div className="fixed inset-0 z-20 flex items-center justify-center p-4 bg-black/40 modal-overlay-enter" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-violet-400 dark:border-violet-900">
          <h2 className="font-semibold text-gray-900 dark:text-gray-100">{t("profile.followRequestsTitle")}</h2>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto p-2">
          ...
          {status === "ready" && list.length === 0 && (
            <p className="p-3 text-sm text-gray-500 dark:text-gray-400">{t("profile.noFollowRequests")}</p>
          )}
          {status === "ready" &&
            list.map((r) => (
              <div key={r.id} className="flex items-center gap-3 p-2 rounded-lg">
                <Link to={`/u/${r.requester.handle}`} onClick={onClose} className="flex items-center gap-3 min-w-0 flex-1">
                  <Avatar name={r.requester.name} picture={r.requester.picture} presetId={r.requester.avatarPreset} size="sm" />
                  <div className="min-w-0">
                    <p className="font-medium text-gray-900 dark:text-gray-100 truncate">{r.requester.name}</p>
                    {r.requester.nickname && <p className="text-xs text-gray-400 truncate">{r.requester.nickname}</p>}
                  </div>
                </Link>
